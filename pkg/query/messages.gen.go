...
func newMessage(db *gorm.DB, opts ...gen.DOOption) message {
	...
	_message.Message = field.NewString(tableName, "message")
	...
	_message.User = messageBelongsToUser{
...
type messageBelongsToUser struct {
	db *gorm.DB

	field.RelationField
}

func (a messageBelongsToUser) Where(conds ...field.Expr) *messageBelongsToUser {
	if len(conds) == 0 {
		return &a
	}
...
func (a messageBelongsToUser) WithContext(ctx context.Context) *messageBelongsToUser {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a messageBelongsToUser) Session(session *gorm.Session) *messageBelongsToUser {
	a.db = a.db.Session(session)
	return &a
}

func (a messageBelongsToUser) Model(m *model.Message) *messageBelongsToUserTx {
	return &messageBelongsToUserTx{a.db.Model(m).Association(a.Name())}
}

type messageBelongsToUserTx struct{ tx *gorm.Association }

func (a messageBelongsToUserTx) Find() (result *model.User, err error) {
	return result, a.tx.Find(&result)
}

func (a messageBelongsToUserTx) Append(values ...*model.User) (err error) {
...
func (a messageBelongsToUserTx) Replace(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a messageBelongsToUserTx) Delete(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a messageBelongsToUserTx) Clear() error {
	return a.tx.Clear()
}

func (a messageBelongsToUserTx) Count() int64 {
	return a.tx.Count()
}
